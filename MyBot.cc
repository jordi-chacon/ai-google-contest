...
bool is_stupid_to_go_to_planet(const PlanetWars& pw, Planet p, int turn) {
  return is_neutral_planet_with_many_fleets(p);
}

void SendShips(const PlanetWars& pw, int my_planet, int my_planet_ships, int turn) {
  ...
  for(vector<Planet>::const_iterator it = not_my_planets.begin();
    ...
    if(!is_stupid_to_go_to_planet(pw, *it, turn)) {
...
int ships_needed_to_defend_planet(const PlanetWars& pw, Planet p) {
...
void DoTurn(const PlanetWars& pw, int turn) {
  ...
  for (int i = 0; i < my_planets.size(); ++i) {
    int ships_needed_to_defend = ships_needed_to_defend_planet(pw, my_planets[i]);
    SendShips(pw, my_planets[i].PlanetID(), ships_needed_to_defend, turn);
  }
}
